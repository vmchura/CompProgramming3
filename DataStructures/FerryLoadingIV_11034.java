 ...
 * @author vchura
 */
public class FerryLoadingIV_11034 {
   public static enum Side{
    ...
    public void solve(){
        ...
        while(casos-->0){
            int sizeFerry=scanner.nextInt()*100;
            int m=scanner.nextInt();
            Queue<Integer> L=new LinkedBlockingQueue<>();
            Queue<Integer> R=new LinkedBlockingQueue<>();
            
            Side currentSide=Side.Left;
            while(m-->0){
                int size_cm=scanner.nextInt();
                String side_str=scanner.next();
                if(side_str.equals("left")){
                    L.add(size_cm);
                }else{
                    R.add(size_cm);
                }
            ...
            while(!L.isEmpty() || !R.isEmpty()){
                ...
                if(currentSide==Side.Left){
                    while(!L.isEmpty() && current+L.peek()<=sizeFerry){
                        current+=L.poll();
                    ...
                    while(!R.isEmpty() && current+R.peek()<=sizeFerry){
                        current+=R.poll();
    ...
    public static void main(String[] args){
        FerryLoadingIV_11034 solver=new FerryLoadingIV_11034();
        solver.solve();       
    }
