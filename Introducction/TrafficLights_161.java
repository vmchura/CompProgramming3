 ...
 * @author vchura
 */
public class TrafficLights_161 {
   ...
   Runtime.getRuntime().exec("gedit "+file_1.getAbsolutePath()+" "+file_2.getAbsolutePath());
    ...
    public void solve(ArrayList<Integer> A){
        boolean[] tiempo=new boolean[60*60*5+1];
        boolean answerPrinted=false;
        for(int t=1;t<60*60*5+1;t++){
            boolean flag=true;
            ...
            if(flag && countFirstTime!=A.size()){
                int horas=t/3600;
                int minutos=(t-horas*3600)/60;
                int segundos=t-horas*3600-minutos*60;
                PS.println(String.format("0%d:%s%d:%s%d",horas,
                                                                    minutos<10?"0":"",
                                                                    minutos,
                                                                    segundos<10?"0":"",
                                                                    segundos));
        ...
        if(!answerPrinted){
            PS.println("Signals fail to synchronise in 5 hours");
        }
    ...
    public static void main(String[] args){
        TrafficLights_161 solver=new TrafficLights_161();
        solver.solve();
    }
