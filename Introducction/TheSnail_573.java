...
Runtime.getRuntime().exec("gedit "+file_1.getAbsolutePath()+" "+file_2.getAbsolutePath());
    ...
    public void solve(){
        while(scanner.hasNext()){           
            ...
            float distanceToClimb=U;
            final float slidePerNight=D;
            final float fatiguePerDay=(U*F)/100.0f;
            
            for(int day=1;;day++){
                if(distanceToClimb>=0.0f)
                    currentHeight+=distanceToClimb;
                if(currentHeight>H){
                    PS.println(String.format("success on day %d",day));
                    break;
                }
                currentHeight-=slidePerNight;
                if(currentHeight<0){
                    PS.println(String.format("failure on day %d",day));
                    break;
                }
                
                distanceToClimb-=fatiguePerDay;
    ...
    public static void main(String[] args){
        TheSnail_573 solver=new TheSnail_573();
        solver.solve();       
    }
}
