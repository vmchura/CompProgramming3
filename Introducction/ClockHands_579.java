...
public class ClockHands_579 {
    ...
    public void solve(){
        while(scanner.hasNext()){
            ...
            if(H+M==0)
                break;
            float angleH=H*30.0f+M*0.5f;
            float angleM=M*6;
            float answer=(angleH>angleM)?angleH-angleM:angleM-angleH;
            answer=(answer+360.0f)%360.0f;
            if(answer>=180.0f) answer=360.0f-answer;
            PS.println(String.format("%.3f",answer));
        }
       closeFiles();
    ...
    public static void main(String[] args){
        ClockHands_579 solver=new ClockHands_579();
        solver.solve();       
    }
}
