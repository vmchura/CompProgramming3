...
Runtime.getRuntime().exec("gedit "+file_1.getAbsolutePath()+" "+file_2.getAbsolutePath());
    ...
    public void solve(){
        while(scanner.hasNext()){
            ...
            int lowestPrice_p=Integer.MAX_VALUE;
            for(int i=0;i<nH;i++){
                int p=scanner.nextInt();
                for(int w=0;w<W;w++){
                    int f=scanner.nextInt();
                    if(f>=N && lowestPrice_p>p){
                       lowestPrice_p=p;
                    }
                }
            }
            if(lowestPrice_p!=Integer.MAX_VALUE && lowestPrice_p*N<=B){
                PS.println(lowestPrice_p*N);
            }else{
                PS.println("stay home");
            }
        }
    ...
    public static void main(String[] args){
        EventPlanning_11559 solver=new EventPlanning_11559();
        solver.solve();       
    }
}
